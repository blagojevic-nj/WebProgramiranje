...
public class KorisniciDAO {
	...
	public Korisnik prijava(String username, String password) {
		if (mapaKorisnika.containsKey(username)) {
			if (mapaKorisnika.get(username).getPassword().equals(password) && !mapaKorisnika.get(username).getBlokiran()
					&& !mapaKorisnika.get(username).getObrisan())
				return mapaKorisnika.get(username);
		}
		return null;
	}
	...
	public Korisnik registracija(Korisnik k) {
		...
		if (k.getUloga() == Uloga.KUPAC) {
			Kupac kupac = new Kupac(k.getUsername(), k.getPassword(), k.getIme(), k.getPrezime(), k.getPol(),
					k.getDatumRodjenja(), k.getUloga(), k.getObrisan(), false, new ArrayList<String>(), 0, -1);

			int id = tipoviKupaca.size() + 1;

			TipKupca tk = new TipKupca(id, k.getUsername(), ImeTipa.BRONZANI, 0, 3000, false);
		...
		// treba uraditi i za prodavca
			...
			Prodavac prodavac = new Prodavac(k.getUsername(), k.getPassword(), k.getIme(), k.getPrezime(), k.getPol(),
					k.getDatumRodjenja(), k.getUloga(), k.getObrisan(), false, new ArrayList<>(), new ArrayList<>());
	...
	public void upisiTipove() {
		ObjectMapper maper = new ObjectMapper();

		try {
			maper.writeValue(Paths.get(putanja + "tipoviKupaca.json").toFile(), tipoviKupaca.values());
		} catch (IOException e) {
			System.out.println("Greska prilikom upisivanja tipova!");
		}
	}
	...
	public void azurirajTip(ArrayList<Karta> karte, int tip) {
		TipKupca tk = tipoviKupaca.get(tip);
		double ukupnaSuma = 0;
		for (Karta k : karte)
			ukupnaSuma += k.getCena();

		int bodovi = (int) Math.round(ukupnaSuma / 1000 * 133);

		((Kupac) mapaKorisnika.get(tk.getUsernameKupca()))
				.setBrojBodova(((Kupac) mapaKorisnika.get(tk.getUsernameKupca())).getBrojBodova() + bodovi);
		if (((Kupac) mapaKorisnika.get(tk.getUsernameKupca())).getBrojBodova() >= tk.getBrojBodova()) {
			if (tk.getImeTipa() == ImeTipa.BRONZANI) {
				tk.setImeTipa(ImeTipa.SREBRNI);
				tk.setBrojBodova(5000);
				tk.setPopust(3);
				upisiTipove();
			} else if (tk.getImeTipa() == ImeTipa.SREBRNI) {
				tk.setImeTipa(ImeTipa.ZLATNI);
				tk.setPopust(5);
				upisiTipove();
			}
		}

		izmena((Kupac) mapaKorisnika.get(tk.getUsernameKupca()));
	}

	public void otkazi(Karta k) {
		Kupac kupac = (Kupac) mapaKorisnika.get(k.getKupac());
		TipKupca tk = tipoviKupaca.get(kupac.getTip());
		int bodovi = (int) Math.round(k.getCena() / 1000 * 133 * 4);
		kupac.setBrojBodova(kupac.getBrojBodova() - bodovi);
		if (tk.getImeTipa() == ImeTipa.ZLATNI && kupac.getBrojBodova() < 5000) {
			tk.setBrojBodova(5000);
			tk.setImeTipa(ImeTipa.SREBRNI);
			tk.setPopust(3);
			upisiTipove();
		...
		if (tk.getImeTipa() == ImeTipa.SREBRNI && kupac.getBrojBodova() < 3000) {
			tk.setBrojBodova(3000);
			tk.setImeTipa(ImeTipa.BRONZANI);
			tk.setPopust(0);
			upisiTipove();
		}
	...
	public Collection<Korisnik> filtriraj(Collection<Korisnik> kolekcija, Collection<String> uslovi, KarteDAO karte) {
		ArrayList<Korisnik> filtrirano = new ArrayList<Korisnik>();
		ArrayList<String> listaUslova = (ArrayList<String>) uslovi;
		boolean sumnjivi, admin, prodavac, kupac, zlatni, srebrni, bronzani;
		sumnjivi = imaUslov("sumnjivi", listaUslova);
		...
		if(sumnjivi) {
			for(Korisnik k: karte.getSumnjiviKorisnici(getKupci())) {
				filtrirano.add(k);
			}
			return filtrirano;
		} 
		...
		if (kupac) {
			for (Korisnik k : kolekcija) {
				// ako je kupac ako nije stavio nista onda sve kupce ako je stavio nesto onda
				// samo njih
				if (k.getUloga() == Uloga.KUPAC) {
